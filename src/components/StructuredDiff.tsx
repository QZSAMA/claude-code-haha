...
function renderColorDiff(patch: StructuredPatchHunk, firstLine: string | null, filePath: string, fileContent: string | null, theme: string, width: number, dim: boolean, splitGutter: boolean): CachedRender | null {
  ...
  if (hit) return hit;
  const diffInstance = new ColorDiff(patch, firstLine, filePath, fileContent);
  if (!diffInstance || typeof diffInstance.render !== 'function') return null;
  const lines = diffInstance.render(theme, width, dim);
